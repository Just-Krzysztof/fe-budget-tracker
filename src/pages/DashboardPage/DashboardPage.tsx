...
export const DashboardPage = () => {
  const data = useMonthySummary(6, 2025);
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
        {Array.isArray(data.monthySummary) && data.monthySummary.map((summary: MonthySummary, index: number) => (
          <div key={index} className="bg-white rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all duration-300 hover:border-slate-300">
            {/* Currency Header */}
              ...
              <div className="mb-4 p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-emerald-800 font-medium">Przychody</span>
                  </div>
              ...
              <div className="mb-4 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-indigo-800 font-medium">Oszczędności</span>
                  </div>
              ...
              <div className="pt-4 border-t border-slate-200">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`font-bold text-xl ${
                    ...
                    {(summary.incomeSum - summary.expenseSum) >= 0 ? '+' : ''}
                    {(summary.incomeSum - summary.expenseSum).toLocaleString()} {summary.currency}
                  </span>
                </div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <h3 className="text-lg font-semibold text-slate-800 mb-4">Szybkie akcje</h3>
        <div className="flex flex-wrap gap-3">
          <button className="bg-indigo-500 hover:bg-indigo-600 text-white px-6 py-3 rounded-xl font-medium transition-all duration-200 flex items-center space-x-2 shadow-sm hover:shadow-md">
            <span>Dodaj transakcję</span>
          </button>
          <button className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-3 rounded-xl font-medium transition-all duration-200 flex items-center space-x-2 shadow-sm hover:shadow-md">
            <span>Nowy cel</span>
          </button>
